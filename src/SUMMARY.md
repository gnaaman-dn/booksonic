# Summary

[Getting Started](./getting_started.md)

# Build-system

- [Caching Framework](./caching_framework.md)
- [Adding a new container](./adding_a_container.md)

# Usage

- [Virtual Switch]()
    - [Deploying SONiC-VS on virsh](./virtual_switch/deploying_vs_on_virsh.md)
    - [Deploying SONiC-VS on Docker](./virtual_switch/deploying_vs_on_docker.md)
    - [Deploying SONiC-VS on ContainerLab](./virtual_switch/deploying_vs_on_containerlab.md)
    - [Link assignment](./virtual_switch/link_assignment.md)
- [CLI Examples](./cli_examples.md)
- [Insights](./insights.md)
- [IXIA](./ixia.md)
- [Configuration](./configuration.md)

# General DevOps Voodoo

- [Local Docker Registry](./local_docker_registry.md)